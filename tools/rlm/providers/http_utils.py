...
def _read_response(request: urllib.request.Request, timeout_s: int) -> tuple[int, str, float]:
    ...
    try:
        ...
        latency_ms = (time.perf_counter() - start) * 1000.0
        return 0, "ValueError: invalid request configuration", latency_ms
    except OSError as exc:
        latency_ms = (time.perf_counter() - start) * 1000.0
        return 0, f"OSError: {exc}", latency_ms


def get_json(url: str, headers: dict[str, str], timeout_s: int) -> tuple[int, str, float]:
