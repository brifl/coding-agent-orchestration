...
- **Global:** `$CODEX_HOME/` (defaults to `~/.codex/`) - This is the user-level directory where Codex skills are installed. This allows for sharing skills across different projects.
- **Repo-Local:** `.codex/` and `.vibe/` - `.codex/skills` stores repo-local skills, while `.vibe/` stores workflow state/configuration.
...
1.  **Repo-Local (`.codex/skills`)**: Skills in the repository's `.codex/skills` directory take the highest precedence. This allows projects to override global skills with their own versions.
2.  **Global (`$CODEX_HOME/skills`)**: If a skill is not found in the repo-local directory, the global Codex directory is checked.
...
- **Global resources (`$CODEX_HOME/`)**: These resources are located in the user's home directory and are not part of any specific repository, so they are not and should not be versioned with the project.
