...
1. **Repo-local** (`.codex/skills/<name>/`) — highest priority
2. **Config skill_folders** (in order listed)
3. **Global user** (`$CODEX_HOME/skills/<name>/`, defaults to `~/.codex/skills`)
4. **System** (`/etc/codex/skills/<name>/`)
...
This is the default generated by `bootstrap.py <repo-path>`. It changes no behavior.
...
- Loads skills from `.codex/skills/` and `tools/deploy-skills/`
